 ...
 * Description   : Heap and Stack Memory Allocation
 *****************************************************************************/
...
void die(const char *message, struct Connection *conn)
{
    ...
    if (conn != NULL)
    {
        // Always close connection if die() is called
        Database_close(conn);
    }
...
struct Connection *Database_open(const char *filename, char mode)
{
    ...
    if (!conn)
    {
        die("Memory error, cannot allocate memory for Connection.", conn);
    }

    conn->db = malloc(sizeof(struct Database));
    if (!conn->db)
    {
        die("Memory error, cannot allocate memory for Database.", conn);
    }
